...
//            int i, j;
    ...
    void sptrsv_csr_levelset(int n, const int *Lp, const int *Li, const double *Lx,
        ...
        for (int l = 0; l < levels; l++) {
            ...
            strstream << "Level " << l << " with " << levelPtr[l + 1] - levelPtr[l] << " tasks";
            timer.new_activity(strstream.str());
        }

        std::ofstream outf;
        std::string filename = std::string("/home/liu/Documents/dev/cholmod-matrices/outputs/tasks_")
                               #ifdef METIS
                               + "metis"
                               #else
                               +"nometis"
        ...
        outf.open(filename);
        if (outf.is_open()) {
            std::cout << "Output tasks timing" << std::endl;
            timer.print(outf);
            outf.close();
        }
